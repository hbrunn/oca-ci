...
RUN case $(lsb_release -c -s) in \
        ...
        jammy|noble) WKHTML_DEB_URL=https://github.com/wkhtmltopdf/packaging/releases/download/0.12.6.1-2/wkhtmltox_0.12.6.1-2.jammy_amd64.deb ;; \
    esac \
...
RUN ODOO_MAJOR=$(echo "$odoo_version" | grep -oE '^[0-9]+' || true) \
    ...
    && if [ "$ODOO_MAJOR" -ge 20 ]; then \
         PM_DEB_URL="https://github.com/odoo/paper-muncher/releases/download/${paper_muncher_version}/paper-muncher_${paper_muncher_version}_$(lsb_release -c -s)_amd64.deb" \
         && curl -fsSL "$PM_DEB_URL" -o /tmp/paper-muncher.deb \
         && apt-get update -qq \
         && DEBIAN_FRONTEND=noninteractive apt-get install -qq -y --no-install-recommends /tmp/paper-muncher.deb \
         && rm /tmp/paper-muncher.deb \
         && ln -snf /opt/paper-muncher/bin/paper-muncher /usr/local/bin/paper-muncher ; \
       else \
         echo "Skipping paper-muncher: not needed for Odoo $odoo_version" ; \
       fi
...
RUN apt-get update -qq \
    && DEBIAN_FRONTEND=noninteractive apt-get install -qq --no-install-recommends \
       ...
       # chrome
       unzip \
       '?and(?name(libatk-bridge.*) | ?name(libatk1.*) | ?name(libdrm2.*) | ?name(libxcomposite1.*) | ?name(libXdamage.*) | ?name(libxfixes3.*) | ?name(libXrandr.*) | ?name(libgbm.*) | ?name(libxkbcommon0.*) | ?name(libpango1.*) | ?name(libcairo2.*) | ?and(?name(libasound2), ?not(?virtual)) | ?name(libasound2t64), ?not(?name(.*-dev)))'
